...
public class ScheduleController {
	...
	public ScheduleController(ScheduleService scheduleService) {
	 ...
	 @GetMapping("/get/{Id}")
	    public ScheduleRequestResponse getAllSchedule(@PathVariable("Id") Long id){
	    	...
	    	Schedule schedule=scheduleService.getBySchId(id);
	    	sourceAirport=restTemplate.getForObject("http://localhost:8000/airport/show/"+schedule.getSourceAirportId(),Airport.class);
	    	
	    	destinationAirport=restTemplate.getForObject("http://localhost:8000/airport/show/"+schedule.getDestinationAirportId(),Airport.class);
    ...
    public ScheduleRequestResponse saveSchedule(@PathVariable("id") Long id,@RequestBody ScheduleRequestResponse scheduleRequestResponse){
    	Airport sourceAirport=new Airport();
    	Airport destinationAirport=new Airport();
    	Schedule schedule=scheduleService.getBySchId(id);
    	restTemplate.put("http://localhost:8000/airport/update/"+schedule.getSourceAirportId(),scheduleRequestResponse.getSourceAirport(),Airport.class);
    	sourceAirport=restTemplate.getForObject("http://localhost:8000/airport/show/"+schedule.getSourceAirportId(),Airport.class);
    	restTemplate.put("http://localhost:8000/airport/update/"+schedule.getDestinationAirportId(),scheduleRequestResponse.getDestinationAirport(),Airport.class);
    	destinationAirport=restTemplate.getForObject("http://localhost:8000/airport/show/"+schedule.getDestinationAirportId(),Airport.class);
